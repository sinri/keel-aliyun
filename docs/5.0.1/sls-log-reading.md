...
println("Progress: "+meta.getProgress());      // "Complete" 或 "Incomplete"
        System.out.

println("Processed rows: "+meta.getProcessedRows());
        System.out.

println("Elapsed: "+meta.getElapsedMillisecond() +"ms");
        System.out.

println("Result count: "+meta.getCount());

// 遍历查询结果
List<SlsQueryResultRow> rows = response.getQueryResultRows();
        for(
SlsQueryResultRow row :rows){
        System.out.

println(row.normalize().
...
println("Query failed: "+err.getMessage());
        });
```
...
> 当 `query` 中包含分析语句（`|` 后的 SQL 部分）时，`line` 和 `offset` 参数无效，需通过 SQL 的 `LIMIT` 语法控制翻页。
...
| `aggQuery`           | String        | 查询语句中 `                                |` 之后的 SQL 部分 |
...
println(row.getString("level") +": "+row.
