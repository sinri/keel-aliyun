...
onSuccess(id ->System.out.

println("MetricRecorder deployed"))
        .
...
accept("http_metrics",latencyRecord);
```
...
println("MetricRecorder stopped"));
```
...
5.0.3 起，`SlsMetricRecorder` 会先复制 `MetricRecord.labels()` 再注入
`source`，不会修改调用方传入的原始 labels map；如果原始 labels 中已经存在
`source`，写入 SLS 的值以 recorder 配置的 source 为准。
